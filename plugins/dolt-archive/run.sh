...
# Auto-discover production databases if DEFAULT_DBS is "auto"
if [[ "$DEFAULT_DBS" == "auto" ]]; then
  AUTO_DBS=$(dolt_query "" "SHOW DATABASES" | grep -v -E '^(information_schema|mysql|dolt_cluster|dolt|testdb_|beads_t|beads_pt|doctest_)$')
  if [[ -z "$AUTO_DBS" ]]; then
