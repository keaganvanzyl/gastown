...
func TestDoctorDogDatabases(t *testing.T) {
	// No config and no server returns nil (dynamic discovery fails gracefully)
	dbs := doctorDogDatabases(nil, "127.0.0.1", 0)
	if dbs != nil {
		t.Errorf("expected nil from failed discovery, got %v", dbs)
	}

	// Custom databases from config override dynamic discovery
	config := &DaemonPatrolConfig{
		Patrols: &PatrolsConfig{
			DoctorDog: &DoctorDogConfig{
				Enabled:   true,
